...
@author: sbush
@contact: stephen.bush@utsouthwestern.edu
'''
...
def protect(path_str, abspath=True):
    ...
    Arguments:
        path_str
        abspath: Boolean denoting whether or not to convert to absolute
            path. Default: True (will prepend with current path iff dirname)
    ...
    if os.path.dirname(val_path) and abspath:
...
def contents(dir_str):
    ...
    if not os.path.exists(dir_str):
        raise IOError("Directory does not exist: '%s'" % dir_str)

    return [os.path.join(dir_str, f) for f in os.walk(dir_str)]


def subdir(dir_str):
    '''Return a list of subdirectories'''
    check = lambda f: not os.path.isfile(f)
    return [f for f in contents(dir_str) if check(f)]
...
def files(dir_str, extn_str='', contains=''):
    '''Return a list of directory contents

    Update: 'extn_str' matching
        Uses os.path.splitext and looks for extn_str
        Used to check end of file name, but limits minor extension
        variations, e.g., .bt2l vs .bt2.
    '''

    file_list = [f for f in contents(dir_str) if os.path.isfile(f)]
    if extn_str:
        file_list = [f for f in file_list
                     if extn_str in os.path.splitext(f)[1]]
    if contains:
...
def walk_safe(dir_str, level=None):
    ...
    Note: Do not use on large directories. You will regret it. Instead,
        use 'walk_safe_generator' directoy.
...
def renew_link(src, dest):
    if os.path.isfile(src):
        if not os.path.islink(src):
            raise OSError('Source must be a symbolic link')
        else:
            os.unlink(src)
    os.symlink(src, dest)
